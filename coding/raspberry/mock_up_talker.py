...
def parse_args():
    parser = argparse.ArgumentParser(description="Serial test script")
    parser.add_argument(
        "--simulate", "-s",
        action="store_true",
        help="Run in simulation (file-based) mode instead of real serial"
    )
    parser.add_argument(
        "--port", "-p",
        default="COM5",
        help="Serial port to use when not simulating (e.g. COM5)"
    )
    parser.add_argument(
        "--baud", "-b",
        type=int,
    ...
    parser.add_argument(
        ...
        choices=['A', 'B'],
        required=True,
        help="Name of this node (A or B) for simulation mode"
    )
